...
thecolumn<-"meansteps"
impData<-data
impData <-impData %>% filter(is.na(steps)) %>% left_join( select(lookup,c(interval,weekday,measteps)),by=c("weekday"="weekday","interval"="interval"))
impData[is.na(steps),"steps"]<-impData[is.na(steps),thecolumn]
